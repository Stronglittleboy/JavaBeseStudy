...
public class Demo3 {
    ...
    private static volatile Queue<VideosVO> resource = new LinkedBlockingQueue<>(6);

    public static void main(String[] args) throws IOException, SQLException, InterruptedException {
        List<Entity> all = Db.use().query("SELECT * FROM t_videos_av WHERE tags LIKE CONCAT('',?,'%') AND id >323 ;", "性爱");
...
//        ThreadPoolExecutor executor = ThreadUtil.newExecutor(2, 2);
    ...
    private static boolean doGetUrl(List<String> list, ChromeDriver webDriver, Entity item) throws InterruptedException {
        ...
        resource.add(new VideosVO(file_name,mp4360Url));
        if (resource.size() == 6){
            resource.notifyAll();
            resource.wait();
        }
